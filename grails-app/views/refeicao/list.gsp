			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="nome" title="${message(code: 'refeicao.nome.label', default: 'Nome')}" />
					
						<g:sortableColumn property="data" title="${message(code: 'refeicao.data.label', default: 'Data')}" />
				...
				<g:each in="${refeicaoInstanceList}" status="i" var="refeicaoInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${refeicaoInstance.id}">${fieldValue(bean: refeicaoInstance, field: "nome")}</g:link></td>
					
						<td><g:formatDate date="${refeicaoInstance.data}" /></td>
